...
export function setupTelegramBot() {
  ...
  const executeBuy = async (userId: string, mint: string, amount: string, chatId: number) => {
    try {
      ...
      await storage.createTrade({ 
        ...
        status: 'completed', 
        txHash: txid,
        tp1: user?.tpPercent || null,
        sl: user?.slPercent || null
      });
  ...
  bot.on('message', async (msg: any) => {
    ...
    // Simple command parsing for bindings and help
    try {
      const parts = text.trim().split(/\s+/);
      const cmd = parts[0].toLowerCase();

      // helper: check if user is chat admin
      ...
      if (cmd === '/bindings' || cmd === '/help_bindings' || cmd === '/help') {
        // Send the bindings doc (short summary) — if too long, send first 4096 chars
        const fs = await import('fs');
        try {
          const md = fs.readFileSync('./docs/bindings.md', 'utf8');
          const chunk = md.slice(0, 4000);
          await bot.sendMessage(chatId, `<b>Bindings Help</b>\n\n${chunk}`, { parse_mode: 'HTML' });
        } catch (e) {
          await bot.sendMessage(chatId, 'Bindings doc not available on server. See docs/bindings.md in the repo.');
        }
        return;
      }

      if (cmd === '/bind') {
        // /bind crypto  OR  /bind topic <lane> OR /bind forex
        const sub = (parts[1] || '').toLowerCase();
        const VALID_LANES = new Set(['high', 'med', 'low', 'unfiltered', 'cto']);

        if (!await isAdmin()) return bot.sendMessage(chatId, 'Only group admins can run /bind.');

        // Bind whole group for crypto
        if (sub === 'crypto') {
          // prevent mixing if group already bound for forex
          const existing = await storage.getGroupBinding(chatId.toString());
          if (existing && existing.market === 'forex') {
            return bot.sendMessage(chatId, 'This group is already bound for FOREX. Unbind it first to bind for CRYPTO.');
          }
          await storage.upsertGroupBinding({ groupId: chatId.toString(), lane: 'unfiltered', market: 'crypto', purpose: 'crypto group' } as any);
          return bot.sendMessage(chatId, `✅ This group is now bound for CRYPTO signals (lane: unfiltered). Use /bind topic <lane> inside a thread to register lanes.`);
        }

        // Bind a forum topic to a specific lane (crypto only)
        if (sub === 'topic') {
          const lane = (parts[2] || 'unfiltered').toLowerCase();
          if (!VALID_LANES.has(lane)) return bot.sendMessage(chatId, `Invalid lane. Valid lanes: high, med, low, unfiltered, cto`);

          // prefer current forum topic thread id if present
          const topicId = msg.message_thread_id ? msg.message_thread_id.toString() : (parts[3] || undefined);
          if (!topicId) {
            const hint = msg.message_thread_id ? `Detected thread id: ${msg.message_thread_id}` : 'No thread id detected';
            return bot.sendMessage(chatId, `Please run this inside a topic thread or provide the topic id: /bind topic <lane> <topic_id>\n\n${hint}`);
          }

          // ensure group isn't bound for forex (topics are only for crypto)
          const groupBinding = await storage.getGroupBinding(chatId.toString());
          if (groupBinding && groupBinding.market === 'forex') {
            return bot.sendMessage(chatId, 'Cannot bind a topic inside a FOREX-bound group. Use a separate group for crypto topics.');
          }

          await storage.upsertGroupBinding({ groupId: chatId.toString(), topicId, lane, market: 'crypto', purpose: `topic ${lane}` } as any);
          const where = msg.message_thread_id ? 'this topic thread' : `topic ${topicId}`;
          return bot.sendMessage(chatId, `✅ Bound ${where} to lane ${lane}. (Thread id: ${topicId})`);
        }

        // Bind whole group for forex (group-level only)
        if (sub === 'forex') {
          const existing = await storage.getGroupBinding(chatId.toString());
          if (existing && existing.market === 'crypto') {
            return bot.sendMessage(chatId, 'This group is already bound for CRYPTO. Unbind it first to bind for FOREX.');
          }
          await storage.upsertGroupBinding({ groupId: chatId.toString(), lane: 'forex', market: 'forex', purpose: 'forex group' } as any);
          return bot.sendMessage(chatId, `✅ This group is now bound for FOREX signals. Forex uses pair+timeframe routing, not topics.`);
        }

        return bot.sendMessage(chatId, 'Usage: /bind crypto | /bind forex | /bind topic <lane> (run inside a forum topic). In supergroups use topics for per-thread lanes.');
      }
      if (cmd === '/unbind') {
        // /unbind [topic] - removes binding for group or topic
        if (!await isAdmin()) return bot.sendMessage(chatId, 'Only group admins can unbind.');
        const target = (parts[1] || '').toLowerCase();
        if (target === 'topic') {
          const topicId = msg.message_thread_id ? msg.message_thread_id.toString() : (parts[2] || undefined);
          if (!topicId) return bot.sendMessage(chatId, 'Provide topic id or run this inside the topic thread: /unbind topic');
          await storage.deleteGroupBinding(chatId.toString(), topicId);
          const where = msg.message_thread_id ? 'this topic thread' : `topic ${topicId}`;
          return bot.sendMessage(chatId, `✅ Unbound ${where} (Thread id: ${topicId})`);
        }
        // default: unbind whole group
        await storage.deleteGroupBinding(chatId.toString());
        return bot.sendMessage(chatId, `✅ Unbound this group from signals.`);
      }

      if (cmd === '/settings') {
        // Show group bindings for this chat (include topic/thread bindings)
        const bindings = await storage.getGroupBindings(chatId.toString());
        if (!bindings || bindings.length === 0) return bot.sendMessage(chatId, 'No bindings for this group. Use /bind to bind.');
        const partsOut = bindings.map(b => {
          const scope = b.topicId ? `Topic ${b.topicId}` : 'Group-wide';
          return `• ${scope} — Lane: ${b.lane} | Market: ${b.market || 'crypto'} | Purpose: ${b.purpose || ''}`;
        });
        return bot.sendMessage(chatId, `<b>Bindings for this group</b>\n\n${partsOut.join('\n')}`, { parse_mode: 'HTML' });
      }
      if (cmd === '/subscribe') {
        // User-level: subscribe this user to auto-buys for the current topic/thread
        const topicId = msg.message_thread_id ? msg.message_thread_id.toString() : (parts[1] || undefined);
        if (!topicId) return bot.sendMessage(chatId, 'Please run /subscribe inside a topic thread or provide the topic id: /subscribe <topic_id>');
        const binding = await storage.getGroupBinding(chatId.toString(), topicId);
        if (!binding) return bot.sendMessage(chatId, 'This topic is not bound to a lane. Ask an admin to run /bind topic <lane> in this thread.');
        await storage.upsertUserSubscription({ userId, groupId: chatId.toString(), topicId, lane: binding.lane, enabled: true } as any);
        return bot.sendMessage(chatId, `✅ Subscribed you to auto-buys from this topic (lane: ${binding.lane}). Make sure Auto-Buy is enabled in /settings and the lane is toggled on.`);
      }

      if (cmd === '/unsubscribe') {
        const topicId = msg.message_thread_id ? msg.message_thread_id.toString() : (parts[1] || undefined);
        if (!topicId) return bot.sendMessage(chatId, 'Please run /unsubscribe inside a topic thread or provide the topic id: /unsubscribe <topic_id>');
        const binding = await storage.getGroupBinding(chatId.toString(), topicId);
        if (!binding) return bot.sendMessage(chatId, 'This topic is not bound to a lane. Nothing to unsubscribe.');
        await storage.deleteUserSubscription(userId, chatId.toString(), topicId, binding.lane);
        return bot.sendMessage(chatId, `✅ Unsubscribed you from this topic (lane: ${binding.lane}).`);
      }
    } catch (cmdErr) {
    ...
    // Check for auto-buy calls from bound groups (prefer topic binding when present)
    const binding = await storage.getGroupBinding(chatId.toString(), msg.message_thread_id?.toString());
    if (binding) {
      ...
      if (mintMatch) {
        ...
        // Find users who subscribed to this topic/group+lane and who have enabled auto-buy for this lane
        const subscribers = await storage.getSubscribersForBinding(chatId.toString(), msg.message_thread_id?.toString(), binding.lane);

        for (const user of subscribers) {
          // ensure the user has master auto-buy on and the lane is toggled in their settings
          if (!user.autoBuyEnabled) continue;
          const userLaneSettings = await storage.getUserLanes(user.id);
          const laneSetting = userLaneSettings.find(l => l.lane === binding.lane);
          if (!laneSetting || !laneSetting.enabled) continue;
          try {
  ...
  bot.onText(/\/forcebound\s+(low|med|high|cto|unfiltered)\s+([a-zA-Z0-9]{32,44})/, async (msg, match) => {
    ...
    try {
      // Prefer subscribers for the given lane/topic
      const subscribers = await storage.getSubscribersForBinding(chatId.toString(), undefined, lane);
