  ...
  try {
    ...
    for (const symbol of shuffled) {
      ...
      if (bias !== "neutral") {
        ...
        log(`Error analyzing ${symbol}: ${e?.message || e}`, "express");
        try {
          if (e?.response) {
            const status = e.response.status;
            const body = typeof e.response.data === 'string' ? e.response.data : JSON.stringify(e.response.data);
            log(`AI provider response for ${symbol}: status=${status} body=${body.slice(0,1000)}`, "express");
          }
        } catch (ee) {
      ...
      if (token && bindings.length > 0) {
        ...
        for (const b of bindings) {
          try {
            ...
            // Don't post forex signals into topic threads (topics are for crypto lanes only)
            if (marketType === 'forex' && b.topicId) continue;
